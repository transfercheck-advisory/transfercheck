...
function isIgnorableSourceLineRefined(rawCourse) {
  const text = normalizeText(rawCourse);
  
  // Base check
  if (!text) return true;
  
  // Refined/more general checks
  if (
    ...
    text.includes("출처") || // removed trailing space to match "[출처:"
    text.includes("직접 확인") || // source notes
    text.includes("공식 미게시") ||
    text.includes("공식 원문") ||
    text.includes("원문") || // matches any quotes
    text.includes("불필요") ||
    ...
    text.includes("이전") || // matches "최대 16 semester-long courses 이전" and "최대 이전:"
    text.includes("전공 선택") ||
    ...
    text.includes("연 평균") ||
    text.includes("지원 자격") || // match "지원 자격:" and "지원자격"
    text.includes("불인정") ||
    text.includes("공통 필수") ||
    text.includes("최소 성적:") ||
...
database.schools.forEach(school => {
  school.majors.forEach(major => {
    ...
    const nonIgnored = rawReqs.filter(r => !isIgnorableSourceLineRefined(r));
    if (nonIgnored.length <= 1) {
      summary.push({
        school: school.name,
        major: major.name,
        reqCount: nonIgnored.length,
        reqs: nonIgnored
      });
    }
  });
...
console.log(`Audited majors. Total schools/majors with <= 1 prerequisites under refined rules: ${summary.length}`);
summary.forEach(item => {
  console.log(`- School: ${item.school}`);
  console.log(`  Major: ${item.major.replace(/\n/g, ' ')}`);
  console.log(`  Prerequisites Count: ${item.reqCount}`);
  console.log(`  List: ${JSON.stringify(item.reqs)}`);
});
