...
function isIgnorableSourceLine(rawCourse) {
  ...
  return (
    ...
    || text.includes("공식 required courses 없음")
  );
}

database.schools.forEach(school => {
  let hasZeroInAllMajors = true;
  let majorCount = school.majors.length;
  let zeroPrereqMajors = [];
  ...
  school.majors.forEach(major => {
    ...
    if (nonIgnored.length > 0) {
      hasZeroInAllMajors = false;
    } else {
      zeroPrereqMajors.push(major.name);
    }
  });

  if (hasZeroInAllMajors) {
    console.log(`[HOLISTIC SCHOOL] ${school.name} (id: ${school.id})`);
    console.log(`  All ${majorCount} majors have 0 prerequisite courses.`);
  } else if (zeroPrereqMajors.length > 0) {
    console.log(`[PARTIAL HOLISTIC SCHOOL] ${school.name} (id: ${school.id})`);
    console.log(`  ${zeroPrereqMajors.length} / ${majorCount} majors have 0 prerequisite courses:`);
    console.log(`  Majors: ${JSON.stringify(zeroPrereqMajors)}`);
  }
});
