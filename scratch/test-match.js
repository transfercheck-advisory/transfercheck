...
const courseCatalogPath = path.join(root, 'course-catalog.js');

// Load files
const transferDataContent = fs.readFileSync(transferDataPath, 'utf8');
const courseCatalogContent = fs.readFileSync(courseCatalogPath, 'utf8');

const sandbox = { window: {} };
vm.createContext(sandbox);
vm.runInContext(transferDataContent, sandbox);
vm.runInContext(courseCatalogContent, sandbox);

const database = sandbox.window.transferDatabase;
const courseCatalog = sandbox.window.courseCatalog;
...
const unmatched = [];
const matchedCount = { total: 0, matched: 0, unmatched: 0 };

database.schools.forEach(school => {
  school.majors.forEach(major => {
    (major.requiredCourses || []).forEach(course => {
      if (isIgnorableSourceLine(course)) return;
      
      matchedCount.total++;
      const matches = canonicalMatches(course);
      if (matches.length === 0) {
...
let outputStr = "";
outputStr += `Summary: Total ${matchedCount.total} courses checked. Matched: ${matchedCount.matched}, Unmatched: ${matchedCount.unmatched}\n`;
outputStr += "\nUnmatched courses list:\n";
const unmatchedGrouped = {};
unmatched.forEach(item => {
  const key = `${item.school} - ${item.course}`;
  if (!unmatchedGrouped[key]) {
    unmatchedGrouped[key] = {
      school: item.school,
      course: item.course,
      majors: []
    };
  }
  unmatchedGrouped[key].majors.push(item.major);
});

Object.values(unmatchedGrouped).forEach(item => {
  outputStr += `[${item.school}] "${item.course}" in majors: ${item.majors.join(', ')}\n`;
});

fs.writeFileSync(path.join(__dirname, 'match-results.txt'), outputStr, 'utf8');
console.log('Results written to match-results.txt');
