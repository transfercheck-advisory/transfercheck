...
database.schools.forEach(school => {
  school.majors.forEach(major => {
    ...
    rawReqs.forEach(raw => {
      // If the line should be ignored under app.js logic, skip
      if (isIgnorableSourceLine(raw)) return;
      
      const matches = canonicalMatches(raw);
      matches.forEach(course => {
        // If the raw text is very long (e.g. > 50 chars) and doesn't look like a direct course name,
        // it might be a false positive match. Let's flag it for inspection.
        const isLongText = raw.length > 50;
        const containsListIndicators = raw.includes(":") || raw.includes("개") || raw.includes("제공") || raw.includes("택");
        if (isLongText || containsListIndicators) {
          console.log(`[POTENTIAL FALSE MATCH] School: ${school.name}, Major: ${major.name}`);
          console.log(`  Raw: "${raw}"`);
          console.log(`  Matched Course: ${course.name} (${course.id})`);
        }
      });
    });
  });
