...
if (harvard) {
  harvard.majors.forEach(major => {
    ...
    const nonIgnored = major.requiredCourses.filter(r => !isIgnorableSourceLine(r));
    console.log(`  Non-ignored count: ${nonIgnored.length}`);
    nonIgnored.forEach(r => console.log(`    - "${r}"`));
  });
}
